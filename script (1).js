...
const CONFIG = {
  ...
  /* Google Form — REAL FORM (your link is already set!) */
  google_form_url:   'https://docs.google.com/forms/d/e/1FAIpQLScPHpE9hfhha96OzjzpDOoaG7uHQjf1PKaqzPr6npBOFQn1wg/formResponse',
  google_form_view:  'https://docs.google.com/forms/d/e/1FAIpQLScPHpE9hfhha96OzjzpDOoaG7uHQjf1PKaqzPr6npBOFQn1wg/viewform',
  google_form_embed: 'https://docs.google.com/forms/d/e/1FAIpQLScPHpE9hfhha96OzjzpDOoaG7uHQjf1PKaqzPr6npBOFQn1wg/viewform?embedded=true',
  /* NOTE: Entry IDs — open your Google Form in browser → right-click
     on any input field → Inspect Element → find name="entry.XXXXXXXXX"
     Replace each entry.0000000XX below with the real numbers from your form */
  google_form_fields: {
...
function renderOrderSuccess(order) {
  ...
  // Build mailto: link pre-filled with ALL customer details
  const mailSubject = encodeURIComponent(`My Order #${order.oid} — TimeVerse.in`);
  const mailBody    = encodeURIComponent(
`Hello TimeVerse.in Team,

I have just placed an order. Here are my complete details:
...
${order.name}`
  ...
  const mailtoLink = `mailto:timeversetimeverse@gmail.com?subject=${mailSubject}&body=${mailBody}`;

  document.getElementById('checkoutContent').innerHTML = `
    <!-- SUCCESS HEADER -->
    <div style="text-align:center;padding:60px 20px 32px;background:linear-gradient(to bottom,var(--dk2),var(--dk))">
      <div class="success-icon" style="margin:0 auto 20px">✓</div>
      <h2 style="font-family:var(--ff-d);font-size:clamp(28px,4vw,42px);margin-bottom:10px">Order Confirmed! 🎉</h2>
      <p style="font-family:var(--ff-b);font-size:17px;color:var(--tx2);max-width:560px;margin:0 auto 24px;line-height:1.8">
        Thank you <strong style="color:var(--gold)">${order.name}</strong>! Your order
        <strong style="color:var(--gold)">#${order.oid}</strong> has been placed.<br>
        Delivery in <strong>3–7 business days</strong>. Pay <strong style="color:var(--gold)">${fmt(order.total)}</strong> on arrival.
      </p>
      ...
      <div style="max-width:520px;margin:0 auto 32px">
        <div class="order-detail-box">
          ...
          <div class="order-detail-row"><span>Total</span><span style="color:var(--gold);font-weight:700">${fmt(order.total)}</span></div>
          <div class="order-detail-row"><span>Payment</span><span>Cash on Delivery</span></div>
          <div class="order-detail-row"><span>Ship To</span><span>${order.city}, ${order.state} - ${order.pin}</span></div>
          <div class="order-detail-row"><span>Delivery ETA</span><span style="color:var(--green)">3–7 Business Days ✓</span></div>
        </div>
      </div>
    </div>

    <!-- ══ STEP 1: SEND EMAIL (MOST IMPORTANT) ══ -->
    <div style="background:rgba(201,168,76,.07);border-top:2px solid var(--gold);border-bottom:2px solid rgba(201,168,76,.3);padding:36px 20px;text-align:center">
      <div style="max-width:680px;margin:0 auto">
        <div style="font-size:32px;margin-bottom:10px">📧</div>
        <h3 style="font-family:var(--ff-d);font-size:24px;color:var(--gold);margin-bottom:10px">
          Step 1 — Send Your Details by Email
        </h3>
        <p style="font-family:var(--ff-b);font-size:16px;color:var(--tx2);line-height:1.8;margin-bottom:22px">
          Click the button below. It will open your email app with your <strong style="color:var(--tx)">Name, Phone Number, Address, and Order details already filled in</strong>.
          Just press <strong style="color:var(--gold)">Send</strong> — and we'll receive everything instantly!
        </p>
        <a href="${mailtoLink}"
           style="display:inline-flex;align-items:center;gap:10px;padding:16px 36px;background:var(--gold);color:var(--dk);font-family:var(--ff-u);font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;border-radius:6px;text-decoration:none;transition:all .3s;box-shadow:0 8px 28px rgba(201,168,76,.35)"
           onclick="this.style.background='#e8c97a'">
          📧 Email My Order Details to TimeVerse →
        </a>
        <p style="font-size:12px;color:var(--tx2);margin-top:14px">
          Sends to: <strong style="color:var(--gold)">timeversetimeverse@gmail.com</strong> · All your details are pre-filled
        </p>
        <p style="font-size:11px;color:var(--tx3);margin-top:6px">
          (If the button doesn't open your email app, manually send an email to <strong>timeversetimeverse@gmail.com</strong> with your Name, Phone, Address and Order ID: <strong style="color:var(--gold)">${order.oid}</strong>)
        </p>
      </div>
    </div>

    <!-- ══ STEP 2: FILL GOOGLE FORM ══ -->
    <div style="padding:44px 20px;text-align:center;background:var(--dk2)">
      <div style="max-width:760px;margin:0 auto">
        <div style="font-size:28px;margin-bottom:10px">📋</div>
        <h3 style="font-family:var(--ff-d);font-size:22px;color:var(--gold);margin-bottom:10px">
          Step 2 — Also Fill This Quick Order Form
        </h3>
        <p style="font-family:var(--ff-b);font-size:16px;color:var(--tx2);line-height:1.8;margin-bottom:26px">
          Please fill in your <strong style="color:var(--tx)">Name, Phone Number, Billing Address, Email and Order details</strong>
          in the form below. This helps us process and track your order correctly.
        </p>

        <!-- GOOGLE FORM IFRAME EMBED -->
        <div style="border:2px solid rgba(201,168,76,.25);border-radius:10px;overflow:hidden;background:#fff;max-width:700px;margin:0 auto 20px">
          <iframe
            src="${CONFIG.google_form_embed}"
            width="100%"
            height="700"
            frameborder="0"
            marginheight="0"
        ...
        <a href="${CONFIG.google_form_view}" target="_blank" rel="noopener"
           style="display:inline-flex;align-items:center;gap:8px;padding:13px 28px;border:1px solid rgba(201,168,76,.4);color:var(--gold);font-family:var(--ff-u);font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;border-radius:6px;text-decoration:none;transition:all .3s">
          📋 Open Form in New Tab ↗
        </a>
        <p style="font-size:11px;color:var(--tx3);margin-top:10px">
          Having trouble viewing the form? Click "Open Form in New Tab" above.
        </p>
      </div>
    </div>

    <!-- BOTTOM ACTIONS -->
    <div style="text-align:center;padding:36px 20px 60px;background:var(--dk)">
      <div style="display:flex;gap:14px;justify-content:center;flex-wrap:wrap">
        <button class="btn btn-gold btn-lg" onclick="nav('home')">Continue Shopping ⌚</button>
        ${STATE.user ? '<button class="btn btn-outline btn-lg" onclick="nav(\'account\')">View My Orders</button>' : '<button class="btn btn-outline btn-lg" onclick="nav(\'login\')">Create Account</button>'}
      </div>
      <p style="font-size:12px;color:var(--tx2);margin-top:20px">
        Questions? Email us at
        <a href="mailto:timeversetimeverse@gmail.com" style="color:var(--gold)">timeversetimeverse@gmail.com</a> ·
        Mon–Sat 10AM–7PM IST
      </p>
    </div>`;
}
...
async function submitToGoogleForm(order) {
  /* Auto-submits order data to your Google Form → Google Sheets.
     NOTE: This works only if you have filled in the correct entry IDs
     in the CONFIG.google_form_fields section above.
     The form itself is also shown to the customer on the confirmation page. */
  ...
  try {
    // Google Forms doesn't support CORS — use no-cors mode (this is expected behaviour)
    ...
    console.log('✅ Order auto-submitted to Google Form');
  } catch (err) {
    console.log('ℹ️ Google Form auto-submit error (customer form on page is the backup):', err);
  }
}
